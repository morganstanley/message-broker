...
- **Connection management**: Connect and disconnect from external systems
...
export class WebSocketAdapter implements IMessageBrokerAdapter<IMyChannels> {
  ...
  // Note: sendMessage now receives the IMessage wrapper created by the broker
  // (the broker will call adapter.sendMessage(message)).
  async sendMessage(message: IMessage): Promise<void> {
    if (this.isConnected()) {
      // You can send the whole message or map it to the external system's shape
...
const adapterId = await broker.registerAdapter(wsAdapter);
...
async sendMessage(message: IMessage): Promise<void> {
  ...
  // Map the broker IMessage into the shape expected by the external system
  await this.externalSystem.send(message.channelName, message);
}
```
